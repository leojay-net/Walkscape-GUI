...
import { GuiContract, PetType } from '@/lib/gui-token';
import {
    TrendingUp,
    Coins,
    Clock,
...
const contract = new GuiContract();
...
export default function Staking() {
    ...
    const loadStakeInfo = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const playerStats = await contract.getPlayerStats(account.address);
            if (playerStats && playerStats.gui_staked > 0) {
                setStakeInfo({
                    amount_staked: BigInt(playerStats.gui_staked),
                    stake_timestamp: Date.now() - 7 * 24 * 60 * 60 * 1000, // Simulate started 1 week ago
                    growth_multiplier: 25, // Using the highest reward rate
                    last_harvest: Date.now() - 14 * 24 * 60 * 60 * 1000 // Simulate last harvest 2 weeks ago
                });
            } else {
                setStakeInfo(null);
            }
    ...
    const handleStake = async () => {
        ...
        setIsStaking(true);
        try {
            const success = await contract.stakeGui(Number(stakeAmount), 604800); // 1 week lock period in seconds
            ...
            if (success) {
                ...
                throw new Error('Staking failed - insufficient balance or error');
            }
        } catch (error) {
    ...
    const handleHarvest = async () => {
        ...
        try {
            const result = await contract.unstakeGui();
            if (result.success) {
                // Simulate pet adoption with the rewards
                await contract.adoptPet(
                    PetType.GUI_PUP,
                    result.rewards
                );
                ...
                setTimeout(() => {
                    ...
                    // Show success message with the rewards
                    alert(`Successfully harvested! Received ${result.rewards.toLocaleString()} GUI tokens and a new pet!`);
                }, 3000);
            } else {
                throw new Error('Harvest failed - no stake found or error');
            }
        } catch (error) {
    ...
    const canHarvest = () => {
        ...
        const stakeDuration = currentTime - Number(stakeInfo.stake_timestamp) / 1000;
        const harvestCooldown = currentTime - Number(stakeInfo.last_harvest) / 1000;
