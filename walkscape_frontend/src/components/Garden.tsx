...
export default function Garden() {
    ...
    const loadGardenData = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            // Placeholder for garden data loading
            setArtifacts([]);
            setPets([]);
        } catch (error) {
    ...
    const handleMintPet = async (petType: PetType) => {
        ...
        setIsMinting(true);
        try {
            // Placeholder for pet minting
            console.log('Minting pet type:', petType);
                ...
                <div className="space-y-4">
                    ...
                    <div className="card-forest">
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-4">
                            {[
                                { type: PetType.GUI_PUP, name: 'GUI Pup', icon: Flower2 },
                                { type: PetType.APTOS_COMPANION, name: 'Aptos', icon: PawPrint },
                                { type: PetType.DIGITAL_FRIEND, name: 'Digital', icon: Trophy }
                            ].map((pet) => {
